...
export default {
  ...
  inject: ['type', 'format', 'value', 'rangeSeparator', 'radius', 'clearable', 'props'],
  data () {
    return {
      isShow: false,
      dayjs: dayjs,
      startTime:  this.value && dayjs(this.value[0]).format(this.format) || null,
      endTime:  this.value && dayjs(this.value[1]).format(this.format) || null,
      // 存储日历的时间列表
      calendar: []
    }
  },

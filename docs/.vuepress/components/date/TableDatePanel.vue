<template>
  <div class="base-calendar__wrapper">
    <div class="calendar-title__wrapper">
      <div class="prev">
        <i
          v-if="picker.yearControler"
          class="iconfont icon-ico-two-left-arrow"
          @click="changeCalendarDate('subtract', 1, 'year')"
        ></i>
        <i
          v-if="picker.monthControler"
          class="iconfont icon-arrowleftb"
          @click="changeCalendarDate('subtract', 1, 'month')"
        ></i>
      </div>
      ...
      <div class="next">
        <i
          ...
          @click="changeCalendarDate('add', 1, 'month')"
        ></i>
        <i
          v-if="picker.yearControler"
          class="iconfont icon-ico-two-right-arrow"
          @click="changeCalendarDate('add', 1, 'year')"
        ></i>
      </div>
    </div>
    <table>
      ...
      <tbody>
        <tr v-for="(row, rowIndex) in calendar" :key="rowIndex">
          ...
          >
            <span>{{dayjs(column.date).date()}}</span>
          </td>
        </tr>
      </tbody>
    </table>
...
export default {
  ...
  methods: {
    ...
    generatCalendar(date = dayjs()) {
      ...
      let lastDateOfLastMonth = curDate
        .subtract(1, "month")
        .endOf("month")
        .date();
      ...
      for (let i = 1, date = lastDateOfLastMonth; i < startDayOfCurMonth; i++) {
        lastMonthDateArr.unshift({
          ...dateObj,
          isDisabled: true,
          date: dayjs(`${curDate.year()}-${curDate.month()}-${date}`).format(
            this.picker.format
          )
        });
        date = date - 1;
      ...
      for (let i = 1; i <= lastDateOfCurMonth; i++) {
        curMonthDateArr.push({
          ...dateObj,
          isDisabled: false,
          date: dayjs(`${curDate.year()}-${curDate.month() + 1}-${i}`).format(
            this.picker.format
          )
        });
      }
      ...
      for (
        let i = 1;
        i <= 42 - startDayOfCurMonth - lastDateOfCurMonth + 1;
        i++
      ) {
        nextMonthDateArr.push({
          ...dateObj,
          isDisabled: true,
          date: dayjs(`${curDate.year()}-${curDate.month() + 2}-${i}`).format(
            this.picker.format
          )
        });
      }
    ...
    changeHoverStatus(time) {
      ...
      for (let i = 0; i < this.calendar.length; i++) {
        for (let j = 0; j < this.calendar[i].length; j++) {
          ...
          if (
            time.length === 2 &&
            dayjs(date.date).isAfter(time[0]) &&
            dayjs(date.date).isBefore(time[1])
          ) {
            date.isHover = true;
          } else {
            date.isHover = false;
